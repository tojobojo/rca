        ...
        for metric in current:
            if metric.get('is_first_run_after_change'):
                ...
                if matching_change:
                    ...
                    abs_delta = abs(change_delta)
                    if abs_delta >= Config.rca.CRITICAL_THRESHOLD_PCT:
                        impact = 'high'
                    elif abs_delta >= Config.rca.WARNING_THRESHOLD_PCT:
                        impact = 'moderate'
                    else:
                        impact = 'low'

                    # Include all correlated changes regardless of threshold
                    # This ensures visibility even if impact is low
                    correlations.append({
                        'step': metric['step_name'],
                        'rule': metric['rule_name'],
                        'current_drop_pct': round(metric['drop_percentage'], 2),
